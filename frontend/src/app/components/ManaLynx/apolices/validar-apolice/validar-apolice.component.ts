...
@Component({
  selector: 'app-validar-apolice',
  templateUrl: './validar-apolice.component.html',
  styleUrls: ['./validar-apolice.component.scss']
})
export class ValidarApoliceComponent implements OnInit {
    id: any;
  ...
  constructor(private route: ActivatedRoute, private apoliceService: ApoliceService, private router: Router, private veiculoService: VeiculoService) { }
    ...
    validarApolice(){
        switch (this.tipoSeguro){
            case 'Pessoal':
                this.apoliceService.validarApolicePessoalSimulacao(this.apoliceDetailsId).subscribe(response => {
                    this.router.navigateByUrl('/#/apolice');
                });
                break;
            case 'Veiculo':
                this.apoliceService.validarApoliceVeiculoSimulacao(this.apoliceDetailsId).subscribe(response => {
                    this.router.navigateByUrl('/#/apolice');
                });
                break;
            case 'Saude':
                this.apoliceService.validarApoliceSaudeSimulacao(this.apoliceDetailsId).subscribe(response => {
                    this.router.navigateByUrl('/#/apolice');
                });
                break;
        }
